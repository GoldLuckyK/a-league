<?php
	...
	function __get_live_score($competition, $season, $round, $game_index, $token) {
		$str_url = "https://www.afl.com.au/api/cfs/afl/matchScore/CD_M".sprintf("$season$competition%02d%02d", $round, $game_index);
	...
	$competitions[] = __get_round_data("014", date("Y"), 4);
	//$competitions[] = __get_round_data("264", date("Y"), 6);
	...
	foreach ($competitions as $competition) {
		...
		foreach ($competition->rounds as $key => $round) {
			...
			$game_obj->game_status = (($round->match->status == "SCHEDULED")?1:(($round->match->status == "Full Time")?3:2));
			...
			if(($game_time < time()) && ($round->match->status != "Full Time")) {
				$check_live = true;
				$live_data = __get_score_data($competition->id, $competition->season, $round->match->roundNumber, $key + 1);
				$game_obj->homeTeam_score = (isset($live_data->homeTeamScore->matchScore->totalScore)?$live_data->homeTeamScore->matchScore->totalScore:0);
				$game_obj->awayTeam_score = (isset($live_data->awayTeamScore->matchScore->totalScore)?$live_data->awayTeamScore->matchScore->totalScore:0);
				$game_obj->homeTeam_behinds = (isset($live_data->homeTeamScore->matchScore->behinds)?$live_data->homeTeamScore->matchScore->behinds:0);
				$game_obj->awayTeam_behinds = (isset($live_data->awayTeamScore->matchScore->behinds)?$live_data->awayTeamScore->matchScore->behinds:0);
				$game_obj->homeTeam_goals = (isset($live_data->homeTeamScore->matchScore->goals)?$live_data->homeTeamScore->matchScore->goals:0);
				$game_obj->awayTeam_goals = (isset($live_data->awayTeamScore->matchScore->goals)?$live_data->awayTeamScore->matchScore->goals:0);
				$game_obj->homeTeam_ladder = 0;
				$game_obj->awayTeam_ladder = 0;
				$game_obj->homeTeam_super_goals = (isset($live_data->homeTeamScore->matchScore->superGoals)?$live_data->homeTeamScore->matchScore->superGoals:0);
				$game_obj->awayTeam_super_goals = (isset($live_data->awayTeamScore->matchScore->superGoals)?$live_data->awayTeamScore->matchScore->superGoals:0);
			} else {
