<?php
	...
	function getRugbyMarket($appKey, $sessionToken, $str_date)
	{
// 1477 - Rugby League
	    $params = '{"filter":{"eventTypeIds":["1477"],
	              "marketCountries":["AU"],
	...
	function generateRugbyMarket($appKey, $sessionToken, $str_date)
	{
		$file_name = "betfair_rugby_api_meetings.".date("Ymd", strtotime($str_date));

		if(@file_exists($file_name)) {
			$meeting_data = json_decode(@file_get_contents($file_name));
			return $meeting_data;
		}
		$meeting_data = getRugbyMarket($appKey, $sessionToken, $str_date);
	...
	function send_odds_to_server($meetings)
	{
		...
		curl_setopt($ch, CURLOPT_URL,"http://racingjapan.com/gtx/scrapper/betfair_rugby_api_odds_import");
		curl_setopt($ch, CURLOPT_POST, 1);
	...
	$marketData = generateRugbyMarket($APP_KEY, $SESSION_TOKEN, date("Y-m-d"));
